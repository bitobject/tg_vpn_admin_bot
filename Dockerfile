...
RUN mkdir -p apps/admin_api apps/core apps/shared apps/telegram_api
...
RUN apt-get update && apt-get install -y openssl ncurses-bin libstdc++6 curl bash ca-certificates
...
COPY --from=builder --chown=app:app /app/_build/prod/rel/telegram_admin_api ./
